...
##### INSTALL FLASH-FM AND REQUIRED SOFTWARES #####
options(scipen=10)
library(flashfm); library(data.table); library(R2BGLiMS); library(parallel)
source("/net/bliss-04-nfs/scratch/jennifer/Uganda/scripts/SW.R")
source("/net/bliss-04-nfs/projects/cardioprj/Experiments/FINE-MAPPING-FEB-21/finemap.R")

# reg indicator
reg.indicator <- read.table('/net/bliss-04-nfs/projects/cardioprj/Experiments/FINE-MAPPING-FEB-21/LoR_FEB21.txt',sep = '\t',fill=T, header=T)
...
traits <- read.table("/net/bliss-04-nfs/scratch/jennifer/Uganda/plots/trait-codes.txt",header=FALSE,as.is=TRUE,fill=TRUE)
ts <- sapply(qt,function(x) traits[which(traits[,1]==x),2])
M <- length(qt)

rel <- read.table("/net/bliss-04-nfs/projects/cardioprj/Gemma Results/JAM/related_samples.txt",header=FALSE)
relids <- as.numeric(substring(rel[,1],first=2))
y <- read.table("/net/bliss-04-nfs/DATA/cardiomet/Uganda/pooled_ug_gwas_wgs_with_raw_and_tfmd_phe_20170622.txt",as.is=TRUE,sep="\t",header=TRUE)
y <- y[,-(1:42)]
y <- y[-relids,qt]
colnames(y)<-ts

covY <- cov(y,use="pairwise.complete.obs")
diag(covY) <- 1 ## Uganda traits are transformed to N(0,1) and mean and var estimates based on unrelated not as good since know truth
Vy <- diag(covY)
ybar <- rep(0,M)
names(ybar) <- ts
names(Vy) <- ts
...
path_FM<-'/net/bliss-04-nfs/projects/cardioprj/Experiments/FINE-MAPPING-FEB-21/FM-FLASHFM/'
...
for (i in 1:M){
  GWAS[[i]] <-read.table(file=paste0("/net/bliss-04-nfs/DATA/cardiomet/Uganda/GWAS/chr.",k,".qt.",aux[i,2],".assoc.txt"),header=TRUE,as.is=TRUE)
...
Ne <- numeric(M)
for(i in 1:M) Ne[i] <- scan(paste0("/net/bliss-04-nfs/DATA/cardiomet/Uganda/GWAS/Neff.qt.",qt[i],".txt"))[2]


##### GENOTYPE MATRIX #####
Gmat<-fread(paste0("/net/bliss-04-nfs/projects/cardioprj/Experiments/FINE-MAPPING-FEB-21/GMAT_FILES/chr",k,'-',
                   reg.indicator$lower.bound[reg],':',reg.indicator$upper.bound[reg],'/Gmat_file.txt'), sep=' ', header = FALSE)
Gmat <- Gmat[,-1]
...
# NOTE: The effective sample size is computed for all the traits in advance using the NEF function included in the R Package: 'flashfm'
...
for(i in 1:M) {
  ...
  #auxiliary<-which((GWAS[[i]]$af>0.01 & GWAS[[i]]$af<0.99))
...
###### Flashfm Input ######
main.input <- flashfm.input(modPP.list=fm,beta1.list=betas,Gmat=X,Nall=Ne,ybar.all=ybar,related=TRUE,y=y, is.snpmat=T)
names(main.input$SM) <- ts
save(main.input, file=paste0(DIRout,'main.input.Rdata'))
print('flashfm.input done')

###### SUMMARY STATS #######
ss.stats <- summaryStats(Xmat=TRUE,ybar.all=ybar,main.input=main.input)
...
if (reg.indicator[reg,8]==4 | reg.indicator[reg,8]==5){
  fm.multi <- flashfm(main.input, TOdds=1,covY=covY,ss.stats=ss.stats,cpp=0.9,maxmod=NULL,fastapprox=FALSE, NCORES=M)
} else {
  fm.multi <- flashfm(main.input, TOdds=1,covY=covY,ss.stats=ss.stats,cpp=0.99,maxmod=NULL,fastapprox=FALSE, NCORES=M)
}
end.time <- Sys.time()
